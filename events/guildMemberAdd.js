...
module.exports = async(member) => {
    ...
    let channel = member.guild.channels.cache.get(ayar.hoşgeldinkanalid)
    let guildID = client.guilds.cache.get(ayar.serverid)
    ...
    if (şüphe === "Güvenli!") {
        member.roles.set([ayar.kayıtsızrolid])
        channel.send(`
\`🔱\` Sunucumuza hoş geldin ${member}!
    
\`🔱\` Hesabın **${moment(member.user.createdTimestamp).format("LLL")}** tarihinde (\`${string}\`) önce oluşturulmuş. 
        
     \`🔱\` Hesap **${şüphe}**

\`🔱\` Sunucu kurallarımız <#${ayar.kurallar}> kanalında belirtilmiştir.
    
\`🔱\` Ayrıca bize destek olmak için sunucuya boost basabilirsiniz.

\`🔱\` Kayıt olmak için <#829782174734745702> kanalına bakabilirsiniz.
        `)
